...
func squared_magnitude(struct_value : felt*, struct_size : felt) -> (res : felt):
    ...
    local res : felt
    if struct_size == 2:
        tempvar x = [struct_value] * [struct_value] 
        tempvar y = [struct_value+1] * [struct_value+1]
        res = x+y
    end
    if struct_size == 3:
        tempvar x = [struct_value] * [struct_value] 
        tempvar y = [struct_value+1] * [struct_value+1]
        tempvar z = [struct_value+2] * [struct_value+2]
        tempvar inter = x+y
        res = inter+z
    end
    return(res = res)
...
# func squared_magnitude(struct_value : felt*, struct_size : felt) -> (res : felt):
#     if struct_size==2:
#         res = struct_value.x * struct_value.x + struct_value.y * struct_value.y
#     else:
#         res = struct_value.x * struct_value.x + struct_value.y * struct_value.y + struct_value.z * struct_value.z
# end
